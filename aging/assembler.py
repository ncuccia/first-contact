...
def withhold_sqr(split_images, ML_labels, label_dic, image_grid_size, log_time = True, norm_split = False, cols = 4, rows = 4):
    ...
    if image_grid_size == 1:
        return random_shuffle(split_images = split_images, ML_labels = ML_labels, label_dic = label_dic,
                            log_time = log_time)
...
def shear_assemble(split_images, ML_labels, label_dic, image_grid_size, length_index, log_time = True):
    
    if not (image_grid_size == 1):
        print('You are using the wrong assemble function.')
        return
    ...
    index = 0
    while np.sum(length_index[:index]) < int(.8* np.sum(length_index)):
        index += 1

    train_index = np.sum(length_index[:index])
    print('Test Index Completed...')

    train_data = split_images[:train_index]
    train_labels = targets[:train_index]
    print('Training Data and Labels Completed...', train_data.shape, train_labels.shape)

    test_data = split_images[train_index:]
    test_labels = targets[train_index:]
    print('Testing Data and Labels Completed...', test_data.shape, test_labels.shape)
    ...
    train_metadata = {key:label_dic[key][:train_index] for key in dic_keys}
    test_metadata = {key:label_dic[key][train_index:] for key in dic_keys}
    print('Training and Testing Metadata Completed...')
    
    return (train_data, train_labels, train_metadata, 
            test_data, test_labels, test_metadata)

def regularize(train_data, test_data):
