...
def data_extractor(files, folders, direc = 'D:/MLdata/', crop_size = 750, split_size = 250, clip_value = 10, subtract = False, log_image = True, younger = 0, unique_shear = False):
    # Intialize Lists
    length_index = []
    split_images = []
    Fn_label = []
    ...
    for file in files:
        ...
        Fs = data['Fs'].flatten()
        if unique_shear:
            temp_shear = np.round(Fs[0],1)
            if temp_shear in np.round(np.array(Fs_label),2):
